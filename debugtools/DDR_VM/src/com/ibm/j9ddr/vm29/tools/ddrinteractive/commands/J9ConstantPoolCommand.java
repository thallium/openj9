...
import com.ibm.j9ddr.vm29.structure.J9Constants;
...
public class J9ConstantPoolCommand extends Command {
	private static final Map<Long, String> cpTypeToString = Map.ofEntries(
		Map.entry(J9ConstantPool.J9CPTYPE_CLASS, "Class"),
		Map.entry(J9ConstantPool.J9CPTYPE_STRING, "String"),
		Map.entry(J9ConstantPool.J9CPTYPE_INT, "Int"),
		Map.entry(J9ConstantPool.J9CPTYPE_FLOAT, "Float"),
		Map.entry(J9ConstantPool.J9CPTYPE_LONG, "Long"),
		Map.entry(J9ConstantPool.J9CPTYPE_DOUBLE, "Double"),
		Map.entry(J9ConstantPool.J9CPTYPE_FIELD, "Fieldref"),
		Map.entry(J9ConstantPool.J9CPTYPE_INSTANCE_METHOD, "Methodref"),
		Map.entry(J9ConstantPool.J9CPTYPE_STATIC_METHOD, "Methodref"),
		Map.entry(J9ConstantPool.J9CPTYPE_INTERFACE_STATIC_METHOD, "Methodref"),
		Map.entry(J9ConstantPool.J9CPTYPE_INTERFACE_INSTANCE_METHOD, "Methodref"),
		Map.entry(J9ConstantPool.J9CPTYPE_HANDLE_METHOD, "HandleMethodref"),
		Map.entry(J9ConstantPool.J9CPTYPE_INTERFACE_METHOD, "InterfaceMethodref"),
		Map.entry(J9ConstantPool.J9CPTYPE_METHOD_TYPE, "MethodType"),
		Map.entry(J9ConstantPool.J9CPTYPE_METHODHANDLE, "MethodHandle"),
		Map.entry(J9ConstantPool.J9CPTYPE_ANNOTATION_UTF8, "UTF8"),
		Map.entry(J9ConstantPool.J9CPTYPE_CONSTANT_DYNAMIC, "ConstantDynamic")
	);
	...
	private static J9RAMClassRefPointer J9VMConstantPoolClassRefAt(J9JavaVMPointer vm, long index) throws CorruptDataException {
		...
		jclConstantPool.addOffset(index);
	...
	private static PointerPointer getSpecialTable(J9ClassPointer ramClass) throws CorruptDataException {
		try {
			...
			Class<?> classPointer = J9ClassPointer.class;
			MethodHandle mhTable;
			if (J9BuildFlags.J9VM_OPT_OPENJDK_METHODHANDLE) {
				mhTable = lookup.findVirtual(classPointer, "invokeCache", MethodType.methodType(PointerPointer.class));
			} else {
				mhTable = lookup.findVirtual(classPointer, "methodTypes", MethodType.methodType(PointerPointer.class));
			}
			...
			for (int i = 1; i < romConstantPoolCount; i++) {
				...
				|| (cpType == J9ConstantPool.J9CPTYPE_INSTANCE_METHOD)
					...
					if (i < ramCPCount) {
						...
						long methodIndex = ramMethodRef.methodIndexAndArgCount().rightShift(METHOD_INDEX_OFFSET).longValue();

						boolean isInitialMethod = true;
						if ((cpType == J9ConstantPool.J9CPTYPE_STATIC_METHOD) ||
							(cpType == J9ConstantPool.J9CPTYPE_INTERFACE_STATIC_METHOD)) {
							if ((ramMethod.getAddress() != initializerMethods.initialStaticMethod().getAddress()) ||
								(methodIndex != J9Constants.J9VTABLE_INITIAL_VIRTUAL_OFFSET)) {
							...
							if (ramMethod.getAddress() != initializerMethods.initialSpecialMethod().getAddress()) {
								...
								if (methodIndex != J9Constants.J9VTABLE_INITIAL_VIRTUAL_OFFSET) { // virtual method
									...
									if (receiverClass.notNull()) {
										long methodAddress = PointerPointer.cast(UDATA.cast(receiverClass).add(methodIndex)).at(0).getAddress();
					...
					if (i < ramCPCount) {
						...
						if (interfaceClass.notNull() && (J9Consts.J9_ITABLE_INDEX_UNRESOLVED != methodIndexAndArgCount.bitAnd(~255).longValue())) {
							UDATA methodIndex = ramMethodRef.methodIndexAndArgCount().rightShift(10);
					...
					try {
						...
						long bsmIndex = romRef.bsmIndexAndCpType().longValue() >> J9NonbuilderConstants.J9DescriptionCpTypeShift & J9NonbuilderConstants.J9DescriptionCpBsmIndexMask;
				...
				CommandUtils.dbgPrint(out, String.format("%-8d%-50s%-50s%-20s%s%n", i, romInfo, ramInfo, cpTypeToString.get(cpType), value));
			}
		} catch (CorruptDataException e) {
