...
import com.ibm.j9ddr.vm29.j9.ContinuationELS;
...
public class StackWalker
{
	 ...
	 */
	public static StackWalkResult walkStackFrames(WalkState walkState, J9VMThreadPointer walkThread)
	{
		return getImpl().walkStackFrames(walkState, walkThread);
	}
	...
	private static interface IStackWalker extends IAlgorithm
	{
		...
		public StackWalkResult walkStackFrames(WalkState walkState, J9VMThreadPointer walkThread);
	...
	private static class StackWalker_29_V0 extends BaseAlgorithm implements IStackWalker
	{
		...
		public StackWalkResult walkStackFrames(WalkState walkState, J9VMThreadPointer walkThread) {
			try {
				...
				if (walkState.walkedEntryLocalStorage == null) {
					walkState.walkedEntryLocalStorage = new ThreadELS(walkThread.entryLocalStorage());
				}
		...
		public StackWalkResult walkStackFrames(WalkState walkState, long continuationAddress) {
			try {
				...
				walkState.walkedEntryLocalStorage = new ContinuationELS(cont);
		...
		public StackWalkResult walkStackFrames(WalkState walkState)
		{
			...
			rc = walkState.privateFlags.anyBitsIn(J9_PRIVATE_FLAGS_STACK_CORRUPT) ? StackWalkResult.STACK_CORRUPT : StackWalkResult.NONE;
			...
			try {
				...
				raiseCorruptDataEvent("CDE thrown extracting initial stack walk state. walkThread = " + walkState.getThreadHexAddress(), ex, true);
				return StackWalkResult.STACK_CORRUPT;
			}
