...
continueTimeCompensation:
	...
	static UDATA waitPolicy = **(UDATA **)omrthread_global((char *)"waitPolicy");
				...
				if (0 == omrthread_monitor_try_enter(vm->cpuUtilCacheMutex)) {
					...
					if (timeDelta >= vm->cpuUtilCacheInterval * 1000000000) {
						...
						if (0 == processTimeRC) {
							...
							double util = (currentProcCPUTimes._systemTime + currentProcCPUTimes._userTime - vm->prevProcCPUTime)
											/ timeElapsedAllCores * 100;
