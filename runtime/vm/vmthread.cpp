	...
	if (JNI_OK == attachSystemDaemonThread(vm, &currentThread, "CPU util calc thread")) {
		...
		while (1) {
			...
			if (vm->prevSysCPUTime.timestamp == 0) {
				...
				Trc_VM_ThreadHelp_timeCompensationHelper_parkMachine(currentThread, currentSysCPUTime.cpuTime, vm->prevSysCPUTime.cpuTime, numberOfCpus, currentTime, vm->prevProcTimestamp);
			}
